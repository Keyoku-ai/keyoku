...
describe("Pulse CLI", () => {
  it("runs the fixture → ingest → plan → audience render path outside the package root", () => {
    const root = mkdtempSync(join(tmpdir(), "keyoku-pulse-cli-"));
    const fixturePath = join(root, "generic.jsonl");
    const project = join(root, "project");
    const timeline = join(root, "timeline.html");
    ...
    expect(plan).toMatchObject({ ok: true, result: { decision: { outcome: "send", checkpointIds: ["cart-recovery-verified"] } } });
    const rendered = JSON.parse(cli(["pulse", "render", "--root", project, "--now", "2026-08-24T16:05:00.000Z", "--debounce-ms", "0", "--audience", "timeline", "--out", timeline, "--json"]));
    expect(rendered).toMatchObject({ ok: true, result: { audience: "timeline", path: timeline } });
    expect(existsSync(timeline)).toBe(true);
    expect(readFileSync(timeline, "utf8")).toContain("Open Factfile evidence");
  });
