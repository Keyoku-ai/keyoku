// Compatibility-only v2 command and MCP entrypoint. This file is built for the
// regression suite but is intentionally excluded from the v3 npm package.
...
const KEYOKU_HOOK_RE = /^(?:node\s+\S*(?:index|legacy-cli)\.js|(?:\S*[/\\])?keyoku)\s+(record|brief|context)\s*$/;
